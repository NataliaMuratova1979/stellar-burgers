...
// --- Сначала функция выполняет HTTP-запрос с помощью fetch и ожидает ответа.
// --- Если запрос успешен, ответ обрабатывается функцией checkResponse, и результат возвращается
// --- Если возникает ошибка и сообщение об ошибке указывает на то, что токен истек ('jwt expired'), выполняется следующее:
// --- Вызывается функция refreshToken(), которая запрашивает новый токен доступа (обычно с использованием refresh-токена).
// --- Новый токен доступа (refreshData.accessToken) обновляет заголовок Authorization в опциях запроса
// --- Выполняется повторный запрос с обновленным токеном.
// --- Если повторный запрос успешен, его ответ также обрабатывается функцией checkResponse, и результат возвращается.
